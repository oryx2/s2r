...
- **OpenAI 兼容**：提供 http://127.0.0.1:8000/v1 标准接口
...
服务启动后，API 地址：`http://127.0.0.1:8000/v1`

### 使用其他模型

```bash
curl -fsSL https://openclaw.ai/install.sh | bash -s -- --model-repo-id Qwen/Qwen2.5-1.5B
```
...
```bash
curl -fsSL https://openclaw.ai/install.sh | bash -s -- --skip-model-check
```
...
- 服务日志：`~/.screen2report/logs/model_server.log`
